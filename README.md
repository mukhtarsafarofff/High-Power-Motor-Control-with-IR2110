...
2) Die VDD-Spannung hängt von der Art der Logik ab, die mit den Eingängen verbunden ist. Ich habe einen 5V-Arduino verwendet, daher habe ich 5V von einem Spannungsregler angeschlossen. Wenn Sie 3V-Logik wie Raspberry Pi GPIO verwenden, schließen Sie VDD an 3V an. Wenn Sie CMOS-Logik mit beispielsweise 12V verwenden, schließen Sie VDD an 12V an.
3) Pin 10 oder HIN steuert den Ausgang an Pin 7, bezeichnet als HO, der den HIGH-Side-MOSFET steuert. Dieser Eingang muss wie unten erläutert pulsweitenmoduliert werden. Pin 12 oder LIN steuert den LOW-Side-MOSFET, der an Pin 1 LO angeschlossen ist. Ein HIGH gibt 10-15V an Pin 1 aus und schaltet den LOW-Side-MOSFET ein. Pin 11 oder SD deaktiviert alle Ausgänge, wenn er auf HIGH gesetzt ist. Dieser ist meistens mit der digitalen Masse verbunden.
